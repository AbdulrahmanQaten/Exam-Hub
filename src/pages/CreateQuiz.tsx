...
export default function CreateQuiz() {
  ...
  const addOption = (qIndex: number) => {
    const updated = [...questions];
    updated[qIndex].options.push({ id: generateOptionId(), text: "" });
    setQuestions(updated);
  };
  ...
  const removeOption = (qIndex: number, oIndex: number) => {
    ...
    if (updated[qIndex].correctOptionId === removedId) {
      updated[qIndex].correctOptionId = "";
    }
    setQuestions(updated);
  };
  ...
  const handleRosterUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (keys.length < 2) {
          toast.error("يجب أن يحتوي الملف على عمودين على الأقل");
          return;
        }

        // If exactly 2 columns, auto-map
        if (keys.length === 2) {
          ...
          setRoster(entries);
          toast.success(`تم استيراد ${entries.length} طالب بنجاح`);
        } else {
  ...
  const handleSave = () => {
    ...
    for (let i = 0; i < questions.length; i++) {
      const q = questions[i];
      if (!q.text.trim()) {
        toast.error(`يرجى كتابة نص السؤال ${i + 1}`);
        return;
      }
      if (!q.correctOptionId) {
        toast.error(`يرجى تحديد الإجابة الصحيحة للسؤال ${i + 1}`);
        return;
      }
      for (const opt of q.options) {
        if (!opt.text.trim()) {
          toast.error(`يرجى كتابة جميع الخيارات في السؤال ${i + 1}`);
          return;
        }
      }
    }
    ...
    if (isEditing) {
      ...
      if (existing) {
        updateQuiz({ ...existing, title, questions, settings, roster: roster.length > 0 ? roster : undefined });
        toast.success("تم تعديل الاختبار بنجاح");
      }
    } else {
      const quiz = createQuiz(title, questions, settings, roster.length > 0 ? roster : undefined);
      ...
      <div className="border-b bg-card">
        <div className="container py-6">
          ...
          <Input
            ...
            className="text-xl font-bold h-14 rounded-xl border-2 focus:border-primary"
          />
        </div>
      </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr,320px]">
          ...
          <div className="space-y-4">
            {questions.map((q, qIndex) => (
              <Card key={q.id} className="p-5 animate-fade-in">
                <div className="flex items-start gap-3 mb-4">
                  <div className="mt-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary text-primary-foreground text-sm font-bold">
                    {qIndex + 1}
                  </div>
                  ...
                  <Button variant="ghost" size="icon" onClick={() => removeQuestion(qIndex)} className="text-destructive hover:text-destructive shrink-0">
                ...
                <div className="mr-11 space-y-2">
                  {q.options.map((opt, oIndex) => (
                    <div key={opt.id} className="flex items-center gap-2">
                      ...
                      {q.type === "mcq" && q.options.length > 2 && (
                        <Button variant="ghost" size="icon" onClick={() => removeOption(qIndex, oIndex)} className="h-8 w-8 shrink-0 text-muted-foreground">
                  ...
                  {q.type === "mcq" && q.options.length < 6 && (
                    <Button variant="ghost" size="sm" onClick={() => addOption(qIndex)} className="gap-1.5 text-muted-foreground">
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={addMCQ} className="flex-1 gap-2 rounded-xl h-14 border-dashed border-2">
                <Plus className="h-5 w-5" />
                اختيار من متعدد
              </Button>
              <Button variant="outline" onClick={addTrueFalse} className="flex-1 gap-2 rounded-xl h-14 border-dashed border-2">
                <Plus className="h-5 w-5" />
                صح أو خطأ
              </Button>
            </div>
            ...
            <Card className="p-5 sticky top-20">
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Label>مؤقت زمني</Label>
                  </div>
                  <Switch
                    checked={settings.timerEnabled}
                    onCheckedChange={(checked) => setSettings({ ...settings, timerEnabled: checked })}
                  />
                </div>
                {settings.timerEnabled && (
                  <div className="flex items-center gap-2 mr-6">
                    <Input
                      ...
                      onChange={(e) => setSettings({ ...settings, timerMinutes: parseInt(e.target.value) || 1 })}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Shuffle className="h-4 w-4 text-muted-foreground" />
                    <Label>ترتيب عشوائي للإجابات</Label>
                  </div>
                  <Switch
              ...
              <div className="mt-6 pt-4 border-t">
                <h4 className="text-sm font-bold mb-3 flex items-center gap-2">
                  ...
                  قائمة الطلاب (اختياري)
                </h4>
                <p className="text-xs text-muted-foreground mb-3">
                  ارفع ملف Excel يحتوي على أسماء وأرقام الطلاب - يمكنك اختيار الأعمدة
                </p>
                <input
                  ref={fileInputRef}
                  type="file"
                ...
                {roster.length > 0 && (
                  <div className="mt-3 p-3 bg-muted rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setRoster([])} className="h-6 text-xs text-destructive hover:text-destructive">
                        إزالة
                      </Button>
                    </div>
                    <div className="max-h-32 overflow-y-auto space-y-1">
                      ...
                      {roster.length > 5 && (
                        <div className="text-xs text-muted-foreground text-center">... و{roster.length - 5} آخرين</div>
                      )}
                    </div>
                  </div>
                )}
              ...
              <div className="mt-4 pt-4 border-t">
                <div className="text-sm text-muted-foreground mb-1">
                  عدد الأسئلة: <strong className="text-foreground">{questions.length}</strong>
                </div>
              </div>

              <Button onClick={handleSave} className="w-full mt-4 gap-2 rounded-xl h-12" disabled={questions.length === 0}>
